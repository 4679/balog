...
local permalink="$(get_config baseurl)$(get_meta $1 slug)/"
local line_start=$(expr $(cat $content_path/post/$file_name | grep -n "+++" | tail -n 1 | cut -f 1 -d :) + 1)
...
mkdir -p $tmp_path/post
cat $content_path/post/$file_name | sed -n "$line_start,999999p" > $tmp_path/post/$file_name.tmp
$work_dir/libs/markdown.sh $tmp_path/post/$file_name.tmp >> $dest_path/index.html
...
sed -i "s/{{ .Title }}/$(get_meta $1 title )/g" $dest_path/index.html
...
sed -i "s/{{ .Date }}/$(format_date $(get_meta $1 date ))/g" $dest_path/index.html
